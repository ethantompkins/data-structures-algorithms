...
void build_graph(void)
{
    ...
    while(current != end)
    {
        ...
        {
            if(current[me] == current[a])
            {
                
                bitset<items> nextstate = current;
                
                nextstate[me].flip();
                
                a != me ? nextstate[a].flip() : false;
                
                if(correctPos(nextstate))
                {
                    int curr = (int)current.to_ulong();
        ...
        current = (int)current.to_ulong() + 1;
    }
}
