...
{
    if(T == NULL)   //make tree if the tree does not exist
    {
        return new Node(v);
    }
    ...
    //find rank of root
    int rank_of_root = T->left ? T->left->size : 0;
    //place node in function to keep tree balanced
    if(rand() % (T->size + 1) == 0)
    {
...
Node *order_n_teams(int n)
{
  ...
  for (int i=1; i<n; i++) 
  {
    ...
    if (did_x_beat_y(select(T,T->size-1)->key, i)) // can we insert at end?
    ...
    else 
    {
	    ...
	    while(lBound != hBound-1)
	    {
	        //find middle of two numbers
	        int middle = (lBound + hBound)/2;
	        //if first number is greater than the second, the lower bound 
	        //becomes the middle
	        if(did_x_beat_y(select(T, middle)->key, i))
	        {
	            lBound = middle;
	        }
	        //the high boundary becomes the middle of the region if not
	        else
	        {
	            hBound = middle;
	        }
	    }
	    T = insert_random(T, i, hBound);    //inserts the team
    }
   }
  }
  return T;
